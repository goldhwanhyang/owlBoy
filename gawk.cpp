...
HRESULT gawk::init(float x, float y, int dir)
{
	...
	SOUNDMANAGER->addSound("고크_날개짓", "SOUND/SoundEffect/gawkFeather.mp3");
	//=====================================
	enemy::init(x, y);
...
void gawk::update()
{
	...
	{
		SOUNDMANAGER->play("날개짓", _soundVolume);
	}
	if (_hp <= 0)
	{
		_state = DEAD;
	...
	case IDLE:
		...
		break;
	case READY:
		//if(_index == 2) SOUNDMANAGER->play("날개짓", _soundVolume);
		//else SOUNDMANAGER->stop("날개짓");
		_y += 1.5;
