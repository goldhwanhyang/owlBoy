...
{	
	...
	bullet blt;
	blt.init(20, 8, 4, IGM->findImage("던전맵")->getWidth(), "토크_불릿");
	for (int i = 0; i < 1; i++)
	{
...
void torque::update()
{
	if (_hp <= 0)
	{
		...
		SOUNDMANAGER->play("적폭발", _effectVolume);
		_isActive = false;
		_hp = 0;
	}
...
void torque::Bmove()
{
	for (int i = 0; i < _vBullet.size(); ++i)
	{
		...
		if (_vBullet[i].getIsActive())
		{
			...
			{
				EFFECTMANAGER->play("돌폭발", _vBullet[i].getX()+50, _vBullet[i].getY()+50);
				SOUNDMANAGER->play("돌폭발", _effectVolume);
			}
		}
	}
}
